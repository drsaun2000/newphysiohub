...
const EditQuizPage = () => {
  ...
  // Fetch main topics for dropdown
  const fetchTopics = async () => {
    try {
      const mainRes = await axios.get(`${SERVER_URL}main-topics`);
      setMainTopics(mainRes.data.data);
    } catch (error) {
  ...
  const fetchQuiz = async () => {
    try {
      const res = await axios.get(`${SERVER_URL}quizzes/${id}`, {
        headers: {
      ...
      setQuiz({
        ...quizData,
        cardTopic: quizData.cardTopic || '',
        thumbnail: quizData.thumbnail || '',
        quizDurationInMinutes: quizData.quizDurationInMinutes || '',
      });
      // Expand all questions by default
      setExpandedQuestions(quizData.questions.map((_, index) => index));
    } catch (error) {
  ...
  const handleSave = async (isDraft = false) => {
    ...
    try {
      ...
      await axios.put(`${SERVER_URL}quizzes/${id}`, quizData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
      toast.success(isDraft ? 'Quiz saved as draft!' : 'Quiz updated successfully!');
      navigate('/quizzes');
    } catch (error) {
